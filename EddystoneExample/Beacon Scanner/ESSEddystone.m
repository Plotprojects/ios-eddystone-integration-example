...
-(NSString*)beaconNamespace{
    return [self getPartOfId:NSMakeRange(1, 2)];
}

-(NSString*)beaconInstance{
    return [self getPartOfId:NSMakeRange(2, 2)];
}

-(NSString*)getPartOfId:(NSRange)range{
    NSString* idAsString = [NSString stringWithFormat:@"%@", self.beaconID];
    NSString* cleanId = [idAsString substringWithRange:NSMakeRange(1, [idAsString length] - 2)];
    NSArray<NSString*>* idParts = [cleanId componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    return [[idParts subarrayWithRange:range] componentsJoinedByString:@""];
}
